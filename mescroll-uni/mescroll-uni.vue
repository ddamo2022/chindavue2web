<template>
	<view class="mescroll-uni-warp">
			 ...
			 @touchend="wxsBiz.touchendEvent" @touchcancel="wxsBiz.touchendEvent" :change:prop="wxsBiz.propObserver" :prop="wxsProp">
				...
				<view class="mescroll-wxs-content" :style="{'transform': translateY, 'transition': transition}" :change:prop="wxsBiz.callObserver"
				 :prop="callProp">
					...
					<view class="downwarp-content" style="z-index: -1;">
		...
		<!-- 回到顶部按钮 (fixedTHB素,需写在scroll-view外面,防止滚动的时候抖动)-->
	...
	export default {
		...
		data() {
			return {
				...
				viewId: 'id_' + Math.random().toString(36).substr(2, 16), // 随机生成mescroll的id(不能数字开头,否则找不到THB素)
		...
		computed: {
			...
			translateY() {
				return this.downHight > 0 ? 'translateY(' + this.downHight + 'px)' : ''; // transform会使fixed失效,需注意把fixedTHB素写在mescroll之外
			},
			// 列表是否可滑动
			scrollable() {
