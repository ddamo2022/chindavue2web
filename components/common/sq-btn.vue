			...
			<view class="pwarp">
				...
				<view class="bf p3 content">
					...
					<view class="f30 con">
						...
						<!-- <view class="c6 f28">加入后享专属活动&会员好礼</view> -->
					</view>
					<!-- #ifdef  MP-WEIXIN -->
				 	 <view class="btn">
						<button :open-type="isAgree?'getPhoneNumber':''" @click="tlogin"
							@getphonenumber="mpGetphonenumber" class="f34 f-c bs60 cf"
							:style="{background:tColor,color:fontColor}">手机号快捷登录</button>
						<view class="f28 c9 f-c mt20" @click.stop="clickNoLogin" v-if="lType==2">暂不登录</view>
					</view>  
					<view style="display: none;">
						<view class="f-s b-b-e pb20">
							...
							<view class="f-g-1"><input type="number" v-model="params.tel" :placeholder="$t('sq-btn.enter_phone_number')" class="f30" /></view>
						</view>
						<view class="f-bt b-b-e pb20 mt20 f-y-c">
							<view class="f-g-1">
								<input type="number" v-model="params.code" :placeholder="$t('sq-btn.enter_verification_code')" class="f30" />
							</view>
							<view class="f-g-0">
						...
						<view class="mt30">
							<button @click="bindmobile" class="f34 f-c bs60 cf"
								:style="{background:params.tel && params.code ? tColor :'#f7f7f7',color:params.tel && params.code ? fontColor :'#999'}">{{$t('sq-btn.login_now')}}</button>
							<!-- <view class="f28 c9 f-c mt20" @click.stop="clickNoLogin" v-if="lType==2">暂不登录</view> -->
						</view>
					</view>
					...
					<view>
						...
						<view class="f-bt b-b-e pb20 mt20 f-y-c">
							...
							<view class="f-g-0">
								<view v-if="time<=0" class="codeBtn f-c f24 c3" @click="sendcode('register')"
									:style="{color:params.tel && params.tel.length>7?fontColor:'',background:params.tel && params.tel.length>7?tColor:''}">
									{{btntxt}}
								</view>
								<view v-else class="codeBtn f-c f24 c3">{{$t('sq-btn.sent')}}  {{btntxt}}</view>
						...
						<view class="mt30">
							<button @click="h5Login" class="f34 f-c bs60 cf"
								:style="{background:params.tel && params.code ? tColor :'#f7f7f7',color:params.tel && params.code ? fontColor :'#999'}">{{$t('sq-btn.login_now')}}</button>
						<!-- 	<view class="f28 c9 f-c mt20" @click.stop="clickNoLogin" v-if="lType==2">暂不登录</view> -->
						</view>
					</view>
	...
	export default {
		...
		methods: {
			...
			open(aid) {
				...
				const currentPath = `/${currentPage.route}${currentPage.options ? '?' + Object.keys(currentPage.options).map(key => `${key}=${currentPage.options[key]}`).join('&') : ''}`;
			...
			tlogin() {
				...
				if (!this.isAgree) {
					this.util.message(`${this.$t('sq-btn.read_and_agree')}${this.basicSetting.agreementTitle || this.$t('sq-btn.user_agreement')}`, 3, 2000)
				} else {
					this.$refs['popup'].close()
				}
			},
					...
					this.util.request({
						...
						if (res.data.phoneNumber) {
							...
							let rest = await this.util.request({
								...
								this.util.message(this.$t('sq-btn.registration_successful'), 1, 1000)
								this.$emit('rf')
							}).catch((e) => {
								this.loading = false
							})
			...
			async h5Login() {
				if (!this.isAgree) {
					return	this.util.message(`${this.$t('sq-btn.read_and_agree')}${this.basicSetting.agreementTitle || this.$t('sq-btn.user_agreement')}`, 3, 2000)
				}
