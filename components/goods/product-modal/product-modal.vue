<template>
	<uni-popup ref="popup" cname="z999" type="bottom" @change="change">
		<view class="pwarp" :style="{paddingBottom:isIpx?'40rpx':'' }">
			...
			<view class="header" v-if="gs && (dType == 'out' && gs.goodsDetail != 2 || dType == 'ins' && gs.goodsInDetail != 2)">
				<button open-type="share" class="s-btn f-y-c">
					<image :src="mrtxImg1"></image>
				</button>
				<image :src="mrtxImg2" @tap="$emit('cancel')"></image>
			</view>
			<scroll-view scroll-y class="content bf">
				<view class="style2 f-bt p2" v-if="gs && (dType == 'out' && gs.goodsDetail == 2 || dType == 'ins' && gs.goodsInDetail == 2)">
					<view class="f-g-0 img bs20"><image :src="ginfo.logo" mode="aspectFill" class="wh" @click="yl(ginfo.logo)"></image></view>
					<view class="f-g-1 ml20">
						<view class="f-bt f-y-t">
							<view class="title f-g-1 t-o-e3">{{ ginfo.name }}</view>
							<view class="f-g-0 flex share l-h1">
								<button open-type="share" class="s-btn f-c">
									<text class="iconfont icon-fenxiang f48 c3" :style="{color:tColor}"></text>
								</button>
								<view class="icon f-c" @tap="$emit('cancel')"><text class="iconfont icon-guanbi2 f40 c3" :style="{color:tColor}"></text></view>
							</view>
						</view>
						<view class="flex f20">
							<view class="mr10 mb10 neutra-font" v-if="orSet.inventoryShow==1 && (dType == 'out' || dType == 'ins')">{{$t("product-modal.inventory")}}:
						...
						<view class="flex f-y-c">
							<view class="flex f-g-0 mr20" v-if="ginfo.discountMinPrice">
								<view class="vipLabel f-bt f22 l-h1">
									<view class="f-g-1 left wei neutra-font">฿
										<text class="f26">{{ginfo.discountMinPrice}}</text>
										<text v-if="ginfo.specSwitch || ginfo.attrSwitch || ginfo.materialSwitch">{{$t("product-modal.from")}}</text>
									</view>
									<view class="f-g-0 right">{{$t("product-modal.member_price")}}</view>
								</view>
							</view>
							<view class="flex mb10" v-if="ginfo.equityCardPrice">
								<view class="vipLabel f-bt f22 l-h1">
									...
									<view class="f-g-0 right f22">
										<text class="neutra-font">{{$t("product-modal.exclusive")}}฿{{ginfo.equityCardPrice}}</text>
										<text v-if="ginfo.specSwitch || ginfo.attrSwitch || ginfo.materialSwitch">{{$t("product-modal.from")}}</text>
									</view>
								</view>
							</view>
							<view class="labels f-g-1" v-if="(ginfo.specSwitch==0 && ginfo.discounts && ginfo.discounts.length) || (ginfo.specSwitch==1 && jsDiscountLabel)">
				...
				<block v-if="gs && (dType == 'out' && gs.goodsDetail != 2 || dType == 'ins' && gs.goodsInDetail != 2)">
					...
					<block v-if="ginfo.materialSwitch==1 && ginfo.materialData.length">
						<view class="materials" style="width: 100%;" v-for="(v, i) in ginfo.materialData" :key="i">
							<view class="p15 c6 f26">{{ v.name }}
								<text class="f20 c9 ml10" v-if="v.required==1">({{$t("product-modal.mandatory")}})</text>
							</view>
							<view class="values">
									...
									<template v-else>
										<template v-if="!cv.b">
											<view class="specsV f-g-1 f-bt ysq">
												...
												<view class="f-g-0">{{$t("product-modal.sold_out")}}</view>
											</view>
										</template>
										<template v-else>
		...
		methods: {
					...
					if (jlkg == 1 && jldata) {
						jldata.forEach(v => {
							if (v.required && !this.xzJlInfo.arr.find(f => f.category.id == v.id)) {
								this.util.message(`${v.name}${this.$t("product-modal.mandatory")}`, 3)
								reject()
							} else {
								reslove()
							}
			...
			async addToCart() {
				...
				if (this.havebxjl) {
					...
					if (num < arr.length) {
						return this.util.message(this.$t("product-modal.select_required_add_ons"), 3, 1500)
					}
				}
