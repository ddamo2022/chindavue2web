<template>
	<uni-popup ref="popup" type="bottom" :sname="{bottom:'172rpx'}" tBom="172" @change="change">
		<view class="cart-popup">
			<view class="ac f-c p1 f24 neutra-font" v-if="cr.discountMoney>0">{{$t("in-cart-popup.discount_applied")}}，{{$t("in-cart-popup.total_discount")}}{{cr.discountMoney}}THB</view>
			<view class="header">
				<view class="order-type">
					<view class="font-weight-bold">{{$t("in-cart-popup.selected_items")}}</view>
					<!-- <view class="extra">自提/外送</view> -->
				</view>
				<view class="flex aict" @tap="clearCar">
					<text class='iconfont icon-shanchu f28 c9 mr10'></text>
					<view>{{$t("in-cart-popup.clear")}}</view>
				</view>
			</view>
			<scroll-view scroll-y class="content">
				<view class="wrapper">
					<view class="list">
						...
						<view>
							<view v-if="cr.boxMoney>0">{{$t("in-cart-popup.packaging_fee")}}：฿{{cr.boxMoney}}</view>
							<view class="f-bt" v-if="cr.tableMoney>0">
								<view class="f-g-1">{{cr.tableFormat || $t("in-cart-popup.service_fee")}}</view>
								<view class="f-g-0 mr30 c9">x{{cr.tableNum}}</view>
								<view class="f-g-0">฿{{cr.tableMoney}}</view>
							</view>
						</view>
					</view>
				</view>
...
<script>
	...
	export default {
		...
		methods: {
			...
			async clearCar() {
				try {
					await this.util.modal(this.$t("in-cart-popup.confirm_cart_clear"))
					await this.clearIncar({
						storeId: this.sjxx.id,
						tableId: this.storeInfo.id,
