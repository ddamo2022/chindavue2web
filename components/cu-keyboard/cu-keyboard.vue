<template>
	<view class="key-container" @click="hide" v-show="showMask">
		<uni-transition :modeClass="['slide-bottom']" :show="show" :styles="{height:'210px'}" :duration="duration">
			<view class="key-content" @click.stop :style="{bottom: isIpx?'40rpx':0}">
				<slot></slot>
				<view class="key-box block flex">
					<view class="key-left">
						...
						<view class="key-bottom">
							<view class="btn-zero">
								<button hover-class="active" class="cu-btn key-btn text-black text-xl"
									@click.stop="keydown('0')">{{type=='people'?'00':'0'}}</button>
							</view>
							<view class="btn-point">
								<button hover-class="active" class="cu-btn key-btn text-black text-xl"
									@click.stop="keydown(type=='people'?'0':'.')">{{type=='people'?'0':'.'}}</button>
							</view>
						</view>
					</view>
					<view class="key-right">
						...
						<view class="confirm">
							<button hover-class="active" :style="{backgroundColor:confirmBtn,color:fontColor}" class="cu-btn paybut"
								@click.stop="confirm">
