...
const clonePayload = (value) => {
  ...
  try {
    ...
    console.warn('Unable to clone payload for cache reuse', error)
    return value
  }
}

const createLayoutCacheKey = (locale, params = {}) => {
  const keyLocale = locale || 'default'
  const page = params.page || ''
  const id = params.id || ''
  return `${keyLocale}::${page}::${id}`
}
...
const resolveLocaleCandidate = (candidate) => {
...
export const useSiteStore = defineStore('site', () => {
  ...
  let queuedLocale = ''
  let activeConfigLocale = lastKnownLocale
  ...
  const loadConfig = async ({ force = false, locale } = {}) => {
    const activeLocaleCandidate = resolveLocaleCandidate(locale)
    const cachedConfig = configCache.get(activeLocaleCandidate)

    if (!force && cachedConfig) {
      config.siteInfo = clonePayload(cachedConfig.siteInfo) || null
      const infoForLayout = config.siteInfo || siteinfo || {}
      const defaultLayoutParams = resolveLayoutParams(infoForLayout, {})
      const layoutKey = createLayoutCacheKey(activeLocaleCandidate, defaultLayoutParams)
      let cachedLayout = clonePayload(cachedConfig.layout)
      if (!cachedLayout && layoutCache.has(layoutKey)) {
        cachedLayout = clonePayload(layoutCache.get(layoutKey))
      }
      config.layout = cachedLayout || null
      activeConfigLocale = activeLocaleCandidate
      lastKnownLocale = activeLocaleCandidate
      ready.value = true
      layoutHydrated.value = Boolean(config.layout)
      applySiteMetadata(config.siteInfo)
      markConnectivityOnline()
      return config.siteInfo
    }

    if (ready.value && !force && activeConfigLocale === activeLocaleCandidate) {
      return config.siteInfo
    }

    if (loading.value) {
      if (force || activeLocaleCandidate !== activeConfigLocale) {
        localeRefreshQueued = true
        queuedLocale = activeLocaleCandidate
      }
      return
    }
    ...
    try {
      ...
      configCache.set(activeLocaleCandidate, {
        siteInfo: clonePayload(config.siteInfo),
        layout: clonePayload(config.layout)
      })

      if (config.layout) {
        const cacheKey = createLayoutCacheKey(activeLocaleCandidate, defaultLayoutParams)
        layoutCache.set(cacheKey, clonePayload(config.layout))
      } else {
        const cacheKey = createLayoutCacheKey(activeLocaleCandidate, defaultLayoutParams)
      ...
      if (!layoutHydrated.value || force) {
        await loadLayout({ force: true, params: defaultLayoutParams, locale: activeLocaleCandidate })
      }
      ...
      if (localeRefreshQueued) {
        const queued = queuedLocale || resolveLocaleCandidate()
        localeRefreshQueued = false
        queuedLocale = ''
        await loadConfig({ force: false, locale: queued })
      }
    }
  }

  const loadLayout = async ({ force = false, params = {}, locale } = {}) => {
    const activeLocaleCandidate = resolveLocaleCandidate(locale)
    const info = config.siteInfo || siteinfo || {}
    const resolvedParams = resolveLayoutParams(info, params)
    const cacheKey = createLayoutCacheKey(activeLocaleCandidate, resolvedParams)

    if (!force) {
      if (layoutCache.has(cacheKey)) {
        config.layout = clonePayload(layoutCache.get(cacheKey))
        layoutHydrated.value = true
        return config.layout
      }

      if (layoutHydrated.value && config.layout && activeConfigLocale === activeLocaleCandidate) {
        return config.layout
      }
    }
    ...
    try {
      const data = await fetchLayoutConfig(info, resolvedParams)
      if (data && typeof data === 'object') {
        config.layout = data
        layoutHydrated.value = true
        layoutCache.set(cacheKey, clonePayload(data))
        if (configCache.has(activeLocaleCandidate)) {
          const cachedEntry = configCache.get(activeLocaleCandidate)
          configCache.set(activeLocaleCandidate, {
            siteInfo: clonePayload(cachedEntry?.siteInfo || config.siteInfo),
            layout: clonePayload(data)
          })
        }
      }
      return config.layout
  ...
  if (typeof window !== 'undefined') {
    const handleLocaleChange = (event) => {
      ...
      loadConfig({ locale: nextLocale }).catch((error) => {
  ...
  const retryConnectivity = async () => {
    await loadConfig({ force: true, locale: activeConfigLocale })
  }
